...
impl Drv8462Config {
    ...
    pub fn as_ctrl3(&self) -> u8 {
        (self.protection.overcurrent_protection_deglitch_time as u8) << 3
            | (self.protection.overcurrent_condition_auto_retry as u8) << 2
            | (self.protection.overtemperature_condition_auto_retry as u8) << 1
    ...
    pub fn as_ctrl4(&self) -> u8 {
        (self.current_regulation.current_sense_blanking_time as u8) << 6
            | (self.step.enable_step_input_filtering as u8) << 2
            | self.step.step_frequency_tolerance as u8
    }
    ...
    pub fn as_ctrl9(&self) -> u8 {
        (self.open_load.enable_open_load_detection as u8) << 7
            | (self.open_load.open_load_immediate_release as u8) << 6
            | (self.open_load.open_load_detection_time as u8) << 4
            | (self.step.dual_step_edge as u8) << 3
            | (self.auto_microstepping.auto_microstepping_resolution as u8) << 1
            | self.auto_microstepping.enable_auto_microstepping as u8
    }
    ...
    pub fn as_ctrl12(&self) -> u8 {
        let fall_time = self.standstill.standstill_fall_time.min(15);

        (self.standstill.standstill_power_saving_mode as u8) << 7 | fall_time << 3
    }

    /// Serializes the CTRL13 fields to be written to the register.
    pub fn as_ctrl13(&self) -> u8 {
        let delay = self.standstill.standstill_delay.clamp(1, 63);
    ...
    pub fn as_ss_ctrl1(&self) -> u8 {
        (self.silent_step.silent_step_sample_time as u8) << 7
            | (self.silent_step.silent_step_frequency as u8) << 3
            | self.silent_step.silent_step_enable as u8
    }

    /// Serializes the SS_CTRL2 fields to be written to the register.
    pub fn as_ss_ctrl2(&self) -> u8 {
        self.silent_step.silent_step_proportional_gain.min(127)
    }

    /// Serializes the SS_CTRL3 fields to be written to the register.
    pub fn as_ss_ctrl3(&self) -> u8 {
        self.silent_step.silent_step_integral_gain.min(127)
    }

    /// Serializes the SS_CTRL4 fields to be written to the register.
    pub fn as_ss_ctrl4(&self) -> u8 {
        (self.silent_step.silent_step_ki_divider_factor as u8) << 6
            | (self.silent_step.silent_step_kp_divider_factor as u8) << 2
    }

    /// Serializes the SS_CTRL5 fields to be written to the register.
    pub fn as_ss_ctrl5(&self) -> u8 {
        self.silent_step.silent_step_transition_frequency.max(1)
    }
}
