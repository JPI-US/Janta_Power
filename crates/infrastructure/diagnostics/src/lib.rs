    ...
    match command {
        ...
        _ => vec![
            ...
            result_line(
                name,
                false,
                &[(String::from("busy"), phase.to_string())],
                "",
            ),
        ],
    }
...
/// What a command needs from the device before it can be answered.
///
/// A statement about the *command*, not about any one firmware: `HDC1080_READ`
/// needs a temperature sensor wherever it runs. A firmware maps these onto
/// whatever owns those things — for a single-threaded runtime that is "the main
/// loop, if it is free"; for a multi-machine one it is a routing table.
///
/// The distinction between [`SharedBus`](CommandNeeds::SharedBus) and
/// [`Sensor`](CommandNeeds::Sensor) is the one worth reading twice, because it is
/// invisible until something runs concurrently. A bus manager locks per
/// *transaction*, which makes a single addressed write safe from any thread. It
/// does nothing for a device sequence spanning several transactions — trigger,
/// wait, read — where another thread can land in the gap and take the reading
/// meant for the first. Those need one owner, not one lock.
///
/// Written as an exhaustive match with no wildcard arm: a command added later
/// will not compile until somebody says what it needs. That matters because
/// getting it wrong fails silently — a command routed to the wrong owner is
/// refused or simply never answered, and neither outcome says why.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CommandNeeds {
    /// Nothing at all. Answerable from the protocol layer, at any moment.
    Nothing,
    /// One self-contained transaction on a shared bus, against a device no other
    /// part of the firmware drives. Safe from any thread.
    SharedBus,
    /// A multi-transaction sequence against a device something else also reads.
    /// Must be performed by whatever owns that device.
    Sensor,
    /// The status indicator.
    StatusLed,
    /// The motor, or the power to it.
    Motor,
    /// Persisted configuration.
    Storage,
    /// A device reset.
    Reset,
    /// Nothing can answer it: the command is unimplemented or unrecognised.
    Unsupported,
}

/// Classify a command by what it needs. See [`CommandNeeds`].
pub fn command_needs(command: &DiagnosticCommand) -> CommandNeeds {
    match command {
        DiagnosticCommand::Ping
        | DiagnosticCommand::FirmwareVersion
        | DiagnosticCommand::GetCapabilities => CommandNeeds::Nothing,

        // Zero-length probes: one addressed write each, no state carried between
        // them. The display is here rather than under `Sensor` because nothing
        // else in the firmware draws to it, so there is no sequence to interleave
        // with.
        DiagnosticCommand::I2cScan | DiagnosticCommand::OledTest { .. } => CommandNeeds::SharedBus,

        // Both are trigger-wait-read against devices the runtime also reads on its
        // own schedule.
        DiagnosticCommand::Hdc1080Read | DiagnosticCommand::RtcCheck => CommandNeeds::Sensor,

        DiagnosticCommand::LedTest { .. } => CommandNeeds::StatusLed,

        DiagnosticCommand::GoHome
        | DiagnosticCommand::MotorMove { .. }
        | DiagnosticCommand::RelayMotor { .. } => CommandNeeds::Motor,
        ...
        | DiagnosticCommand::WriteEnvFile => CommandNeeds::Storage,

        DiagnosticCommand::Reboot => CommandNeeds::Reset,

        // `RELAY_HOTSPOT` has no pin assigned in any firmware yet, so there is no
        // owner to route it to. `Unknown` is a typo, and saying "not supported"
        // sends someone to check their spelling rather than to wait.
        DiagnosticCommand::RelayHotspot { .. } | DiagnosticCommand::Unknown { .. } => {
            CommandNeeds::Unsupported
        }
    }
}
...
pub const CONFIG_KEYS: &[ConfigKey] = &[
    ...
    ConfigKey {
        protocol: "wifi.password",
        nvs: "wifi_pass",
        kind: ConfigValueKind::Text,
        secret: true,
    },
    ConfigKey {
        protocol: "location.latitude",
        nvs: "tower_latitude",
        kind: ConfigValueKind::Latitude,
        secret: false,
    },
    ConfigKey {
        protocol: "location.longitude",
        nvs: "tower_longitude",
        kind: ConfigValueKind::Longitude,
        secret: false,
    },
    ConfigKey {
        protocol: "location.altitude",
        nvs: "tower_altitude",
        kind: ConfigValueKind::Altitude,
        secret: false,
    },
    ConfigKey {
        protocol: "location.timezone_offset_hours",
        nvs: "tz_offset_h",
        kind: ConfigValueKind::TimezoneOffsetHours,
        secret: false,
    },
];
    ...
    F: FnMut() -> (f32, f32),
{
    ...
    match sensor.read_temperature_humidity() {
        Ok((temp, hum)) => {
            let _ = io.write_line(&format!(
                "HDC1080 Correct read: temp:{:.2} hum:{:.2}",
                temp, hum
            ));
        }
        Err(_) => {
...
impl<Board, Environment, Configuration>
    StandardDiagnosticHandler<Board, Environment, Configuration>
{
...
{
    type Error = StandardDiagnosticError<Board::Error, Environment::Error, Configuration::Error>;
...
mod tests {
    ...
    #[test]
    fn every_command_is_classified_by_what_it_needs() {
        use CommandNeeds::*;

        let expected: &[(&str, CommandNeeds)] = &[
            ("PING", Nothing),
            ("FIRMWARE_VERSION", Nothing),
            ("GET_CAPABILITIES", Nothing),
            ("I2C_SCAN", SharedBus),
            ("OLED_TEST BORDER", SharedBus),
            ("HDC1080_READ", Sensor),
            ("RTC_CHECK", Sensor),
            ("LED_TEST RED", StatusLed),
            ("GO_HOME", Motor),
            ("MOTOR_MOVE 10", Motor),
            ("RELAY_MOTOR ON", Motor),
            ("CONFIG_MODE", Storage),
            ("SET_ENV wifi.ssid=site", Storage),
            ("SET_ENV nonsense", Storage),
            ("SAVE_CONFIG", Storage),
            ("GET_ENV device.tower_id", Storage),
            ("GET_CONFIG", Storage),
            ("WRITE_ENV_FILE", Storage),
            ("REBOOT", Reset),
            ("RELAY_HOTSPOT ON", Unsupported),
            ("NONSENSE", Unsupported),
        ];

        for (line, needs) in expected {
            assert_eq!(
                command_needs(&DiagnosticCommand::parse(line)),
                *needs,
                "{line} is classified wrongly"
            );
        }
    }

    #[test]
    fn a_multi_transaction_sensor_is_never_treated_as_bus_safe() {
        // The distinction this enum exists for. Both sensors are trigger-wait-read
        // against devices the runtime also polls on its own schedule, so a second
        // reader landing in the gap takes the first one's reading. Classifying
        // either as `SharedBus` would say "any thread may do this", which is the
        // one thing that is not true of them.
        for line in ["HDC1080_READ", "RTC_CHECK"] {
            let needs = command_needs(&DiagnosticCommand::parse(line));
            assert_eq!(needs, CommandNeeds::Sensor, "{line}");
            assert_ne!(needs, CommandNeeds::SharedBus, "{line}");
        }
    }

    #[test]
    fn nothing_that_moves_the_tower_is_answerable_without_the_motor() {
        for line in ["GO_HOME", "MOTOR_MOVE 10", "RELAY_MOTOR ON"] {
            assert_eq!(
                command_needs(&DiagnosticCommand::parse(line)),
                CommandNeeds::Motor,
                "{line} must be routed to whatever owns the motor"
            );
        }
    }
    ...
    fn an_unrecognised_command_is_not_reported_as_busy() {
        ...
        assert!(!requires_exclusive_runtime(&DiagnosticCommand::parse(
            "NONSENSE"
        )));
    }
    ...
    fn boot_phase_answers_hardware_free_commands_and_refuses_the_rest() {
        ...
        assert_eq!(
            ...
            vec![
                caps,
                "RESULT GET_CAPABILITIES PASS capabilities=SERIAL,STATUS,PING"
            ]
        );
        ...
        for line in [
            ...
            assert!(
                answered[1].starts_with("RESULT ")
                    && answered[1].contains(" FAIL busy=waiting for Wi-Fi"),
    ...
    fn led_test_colors_match_the_installer() {
        ...
        assert_eq!(
            led_test_color("1,2,3,4"),
            None,
            "a dropped field would mislead"
        );
    ...
    /// at again. Provisioning them was work an installer did for no effect, and a
    /// config table listing keys that are always empty makes a fully provisioned
    /// tower look half done.
    ...
    fn config_table_covers_exactly_what_the_installer_sends() {
        for key in INSTALLER_KEYS {
            assert!(
                config_key(key).is_some(),
                "installer sends {key} but the board rejects it"
            );
        }
        assert_eq!(
            CONFIG_KEYS.len(),
    ...
    fn config_values_are_validated_against_their_kind() {
        ...
        assert_eq!(
            check("location.latitude", "90.1"),
            Err(ConfigError::OutOfRange)
        );
        assert_eq!(
            check("location.latitude", "north"),
            Err(ConfigError::NotANumber)
        );
        assert_eq!(check("location.longitude", "-180"), Ok(()));
        assert_eq!(
            check("location.longitude", "180.5"),
            Err(ConfigError::OutOfRange)
        );
        assert_eq!(check("location.timezone_offset_hours", "-6"), Ok(()));
        assert_eq!(
            check("location.timezone_offset_hours", "15"),
            Err(ConfigError::OutOfRange)
        );
        assert_eq!(
            check("location.timezone_offset_hours", "1.5"),
            Err(ConfigError::NotANumber)
        );
        ...
        assert_eq!(
            check("wifi.password", SECRET_SET),
            Err(ConfigError::SecretPlaceholder)
        );
        assert_eq!(
            check("wifi.password", SECRET_UNSET),
            Err(ConfigError::SecretPlaceholder)
        );
    ...
    fn staging_replaces_by_key_and_empties_on_take() {
        ...
        assert!(
            staging.is_empty(),
            "a committed SAVE_CONFIG must not leave values behind"
        );
    }
    ...
    fn static_configuration_builder_writes_expected_sections() {
        let config = StaticDiagnosticConfiguration::new("9.9.9")
            ...
            .with_section(
                DiagnosticConfigSection::new("device").with_entry("tower_id", "TOWER_007"),
            )
            .with_section(
